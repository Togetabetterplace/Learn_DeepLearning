...
def train_model(net, train_iter, test_iter, num_epochs, lr, device):
    ...
    plt.plot(range(num_epochs), train_loss_list, 'ro'， label='Train Loss')
    plt.plot(range(num_epochs), train_acc_list, 'bo', label='Train Acc')
    plt.plot(range(num_epochs), test_acc_list, 'go', label='Test Acc')
    plt.xlabel('Epoch')
    plt.ylabel('Metric')
    plt.legend()
    plt.show()
